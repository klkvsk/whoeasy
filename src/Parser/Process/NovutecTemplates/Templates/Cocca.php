...
class Cocca extends Regex
{
     ...
     * @var array
     */
    protected array $blocks = [ 1 => '/query:(.*?)(?=registrar name)/is',
    ...
    protected array $blockItems = [
        ...
        2 => [ '/registrar name:(?>[\x20\t]*)(.+)$/im'            => 'registrar:name',
        ...
        3 => [ '/name:(?>[\x20\t]*)(.+)$/im'                   => 'contacts:owner:name',
        ...
        4 => [ '/name:(?>[\x20\t]*)(.+)$/im'                   => 'contacts:admin:name',
        ...
        5 => [ '/name:(?>[\x20\t]*)(.+)$/im'                   => 'contacts:tech:name',
        ...
        6 => [ '/name:(?>[\x20\t]*)(.+)$/im'          => 'contacts:billing:name',
     ...
     */
    protected ?string $available = '/(Status: Not Registered|Domain does not exist)/i';
    ...
    public function postProcess(object $WhoisParser): void
    {
        ...
        foreach ($ResultSet->contacts as $contactType => $contactArray) {
            foreach ($contactArray as $contactObject) {
                $contactObject->email = str_ireplace([ ' at ', ' dot ' ], [ '@', '.' ], $contactObject->email);
            }
        }

        if (isset($ResultSet->registrar->email) && $ResultSet->registrar->email != '') {
            $ResultSet->registrar->email = str_ireplace([ ' at ', ' dot ' ], [ '@', '.' ], $ResultSet->registrar->email);
        }
    }
}