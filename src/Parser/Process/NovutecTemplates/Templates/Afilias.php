...
class Afilias extends Regex
{
    ...
    protected array $blockItems = [
        ...
        7 => [ '/^CED ID(?>[\x20\t]*):(?>[\x20\t]*)(.+)$/im'                     => 'contacts:ced:handle',
        ...
        8 => [ '/^Registration Date(?>[\x20\t]*):(?>[\x20\t]*)(.+)$/im' => 'created',
               ...
               '/^DNSSEC(?>[\x20\t]*):(?>[\x20\t]*)(.+)$/im'            => 'dnssec' ],
        9 => [ '/^Zone ID(?>[\x20\t]*):(?>[\x20\t]*)(.+)$/im'                     => 'contacts:zone:handle',
     ...
     */
    protected ?string $available = '/(Status: free|no matching record)|(DOMAIN NOT FOUND|NOT FOUND)|(Domain Status: Available)/i';
    ...
    public function postProcess(object $WhoisParser): void
    {
        ...
        if (is_array($ResultSet->nameserver ?? null)) {
